...
|---|---|---|---|---|---|
| C₁ | Ontological | 0 | $\exists \Lambda$ | There exists a domain | Something exists |
| C₂ | Ontological | 0 | $\forall x: x = x$ | For all x, x is identical to itself | Identity holds |
| C₃ | Ontological | 0 | $\exists x, y: x \neq y$ | There exist x and y such that x differs from y | Multiplicity exists |
| C₄ | Ontological | 1 | $\forall x, t₁, t₂: t₁ < t₂ \implies$ persistent$(x, t₁, t₂)$ | For all x and times t₁, t₂, if t₁ precedes t₂, then x persists from t₁ to t₂ | Persistence |
| C₅ | Ontological | 1 | $\exists \Delta: \Delta \neq \text{id}$ | There exists a transformation that is not the identity function | Transformability |
| C₆ | Ontological | 1 | $\exists p: \neg\text{actual}(p) \wedge \text{possible}(p)$ | There exists p such that p is not actual but p is possible | Potentiality |
| C₇ | Ontological | 1 | $\exists C: \text{constraint}(C) \wedge \text{bounded}(\text{possibility}, C)$ | There exists a constraint C such that C bounds possibility | Constraint |
| C₈ | Ontological | 1 | $\text{consistent}(\Lambda)$ | The domain is internally consistent | Self-containment |
| C₉ | Ontological | 1 | $\forall x, y: x \neq y \implies \exists P: P(x) \wedge \neg P(y)$ | For all x, y, if x differs from y, then there exists a property P that x has and y lacks | Individuation |
| C₁₀ | Ontological | 1 | $\forall x: \exists y: \text{grounds}(y, x) \vee \text{self-ground}(x)$ | For all x, there exists y that grounds x, or x grounds itself | Dependency |
| C₁₁ | Logical | 2 | $\forall x: x = x$ | For all x, x equals x | Identity (logical) |
| C₁₂ | Logical | 2 | $\forall x, y: x \neq y$ | For all distinct x and y, x is not identical to y | Difference (logical) |
| C₁₃ | Logical | 2 | $\Omega_0(\varphi \wedge \neg\varphi) \to g_1^+$ | Frame 0 contains both φ and not-φ, which blooms into frame 1 with enhanced coherence | Metabolic non-contradiction |
| C₁₄ | Logical | 2 | $g_0, g_1, g_2, \ldots$ defined | Truth-value frames g₀, g₁, g₂, etc. are well-defined | Excluded middle (qualified) |
| C₁₅ | Logical | 2 | $\text{meaning}(\phi_1 \wedge \phi_2) = f(\text{meaning}(\phi_1), \text{meaning}(\phi_2))$ | The meaning of φ₁ and φ₂ is a function of the meanings of φ₁ and φ₂ | Compositionality |
| C₁₆ | Logical | 2 | $\neg(\varphi \wedge \neg\varphi)$ within frame | It is not the case that both φ and not-φ hold within a single frame | Non-contradiction (classical) |
| C₁₇ | Logical | 2 | $\text{system}(S) \implies \text{reference}(S, S)$ coherent | If S is a system, then S can coherently refer to itself | Reflexivity |
| C₁₈ | Logical | 2 | $\varphi \wedge (\varphi \to \psi) \implies \psi$ | If φ holds and φ implies ψ, then ψ holds | Inference closure |
| C₁₉ | Logical | 2 | $\forall D: \exists L: \text{represent}(L, D)$ | For every domain D, there exists a language L that represents D | Formal adequacy |
| C₂₀ | Logical | 2 | $\text{meaning}(\phi) \neq \text{extension}(\phi)$ allowed | The meaning of φ may differ from the extension of φ | Intentionality |
| C₂₁ | Temporal | 3 | $\forall t₁, t₂: t₁ < t₂ \vee t₁ = t₂ \vee t₂ < t₁$ | For all times t₁ and t₂, either t₁ precedes t₂, or they are equal, or t₂ precedes t₁ | Temporality |
| C₂₂ | Temporal | 3 | $\forall e: \text{event}(e) \wedge \text{contingent}(e) \implies \exists c: \text{cause}(c, e)$ | For all events e, if e is contingent, then there exists a cause c of e | Causality |
| C₂₃ | Temporal | 3 | $\neg \exists T: \text{bijective}(T)$ | There does not exist a bijective temporal reversal function | Irreversibility |
| C₂₄ | Temporal | 3 | $\text{process}(p) \implies \text{iterate}(p, p)$ possible | If p is a process, then iterating p on itself is possible | Recursion |
| C₂₅ | Temporal | 3 | $\exists S: \text{encode}(S, \text{past}) \subseteq \text{state}(S, \text{now})$ | There exists a system S such that encoded past information is contained in S's present state | Memory |
| C₂₆ | Temporal | 3 | $\text{agent}(A) \implies \text{project}(A, \text{future})$ | If A is an agent, then A can project into the future | Anticipation |
| C₂₇ | Temporal | 3 | $\text{temporal}(s, t) \implies \text{continuous}(s, t)$ | If state s evolves to time t, then this evolution is continuous | Continuity |
| C₂₈ | Temporal | 3 | $P_{\text{macro}} \not\subseteq P_{\text{micro}}$ | Macro-level properties are not a subset of micro-level properties | Emergence |
| C₂₉ | Relational | 4 | $\exists x, y: \text{spatial-relation}(x, y)$ | There exist x and y such that x and y stand in spatial relation | Spatiality |
| C₃₀ | Relational | 4 | $\exists R: \text{symmetric}(R) \vee \text{asymmetric}(R)$ | There exists a relation R that is either symmetric or asymmetric | Symmetry/Asymmetry |
| C₃₁ | Relational | 4 | $\exists L₁, L₂: L₁ \prec L₂$ | There exist levels L₁ and L₂ such that L₁ is lower than L₂ | Hierarchy |
| C₃₂ | Relational | 4 | $\forall x, y: \exists \text{path}(x, y)$ | For all x and y, there exists a path from x to y | Network connectivity |
| C₃₃ | Relational | 4 | $\forall S: \exists B: \text{boundary}(S, B)$ | For every system S, there exists a boundary B of S | Boundary definition |
| C₃₄ | Relational | 4 | $\exists S, E: \text{exchange}(S, E)$ possible | There exist system S and environment E such that exchange between them is possible | Integration (permeability) |
| C₃₅ | Relational | 4 | $\exists M₁, M₂: \text{separable}(M₁, M₂)$ | There exist modules M₁ and M₂ that are separable | Modularity |
| C₃₆ | Relational | 4 | $\forall x, y: \text{relation}(x, y) \implies \text{relation}(y, x)$ | For all x and y, if x relates to y, then y relates to x | Reciprocal determination |
| C₃₇ | Relational | 4 | $\text{synthesis}(\{x_i\}) \neq \bigcup \{x_i\}$ | The synthesis of elements xᵢ is not equal to their simple union | Disjunctive synthesis |
| C₃₈ | Epistemic | 5 | $\forall D: \exists \text{structure}(D)$ | For every domain D, there exists a structure of D | Intelligibility |
| C₃₉ | Epistemic | 5 | $\forall x: \exists O: \text{observe}(O, x)$ possible | For all x, there exists an observer O such that O observing x is possible | Observability |
| C₄₀ | Epistemic | 5 | $\forall D: \exists M: \text{model}(M, D)$ | For every domain D, there exists a model M of D | Modelability |
| C₄₁ | Epistemic | 5 | $\exists A₁, A₂: \text{shared-understanding}(A₁, A₂)$ | There exist agents A₁ and A₂ who share understanding | Intersubjectivity |
| C₄₂ | Epistemic | 5 | $\forall x: \exists p: \text{perceive}(p, x)$ possible | For all x, there exists a perceiver p such that p perceiving x is possible | Perceptual access |
| C₄₃ | Epistemic | 5 | $\exists \mathcal{C}: \text{conceptual-scheme}(\mathcal{C})$ | There exists a conceptual scheme C | Conceptual schemes |
| C₄₄ | Epistemic | 5 | $\forall \phi: \text{truth-apt}(\phi)$ | For all propositions φ, φ is truth-apt | Truth-aptness |
| C₄₅ | Epistemic | 5 | $\forall K: \text{knowledge}(K) \implies \text{fallible}(K)$ | For all knowledge K, if K is knowledge, then K is fallible | Epistemic humility |
| C₄₆ | Semantic | 6 | $\forall s: \exists r: \text{refers-to}(s, r)$ | For all symbols s, there exists a referent r such that s refers to r | Reference |
| C₄₇ | Semantic | 6 | $\forall x, P: \text{predicate}(P, x)$ possible | For all x and predicates P, predicating P of x is possible | Predication |
| C₄₈ | Semantic | 6 | $\text{meaning}(\phi \circ \psi) = f(\text{meaning}(\phi), \text{meaning}(\psi))$ | The meaning of φ composed with ψ is a function of the meanings of φ and ψ | Semantic compositionality |
| C₄₉ | Semantic | 6 | $\text{meaning}(\phi, c₁) \neq \text{meaning}(\phi, c₂)$ possible | The meaning of φ in context c₁ may differ from its meaning in context c₂ | Context-sensitivity |
| C₅₀ | Semantic | 6 | $\forall L₁, L₂: \exists T: \text{translate}(L₁, L₂, T)$ | For all languages L₁ and L₂, there exists a translation T from L₁ to L₂ | Translation |
| C₅₁ | Semantic | 6 | $\exists u: \text{utterance}(u) \implies \text{action}(u)$ | There exists an utterance u such that u is also an action | Performativity |
| C₅₂ | Semantic | 6 | $\text{[Reserved for future semantic condition]}$ | [Reserved for future semantic condition] | [Reserved] |
| C₅₃ | Normative | 7 | $\exists v₁, v₂: v₁ \succ v₂$ | There exist values v₁ and v₂ such that v₁ is better than v₂ | Axiological distinction |
| C₅₄ | Normative | 7 | $\exists A: \text{agent}(A) \wedge \text{chooses}(A)$ | There exists an agent A such that A makes choices | Agency |
| C₅₅ | Normative | 7 | $\forall A, a: \text{agent}(A) \wedge \text{action}(a, A) \implies \text{responsible}(A, a)$ | For all agents A and actions a, if A performs a, then A is responsible for a | Responsibility |
| C₅₆ | Normative | 7 | $\text{freedom}(A) \iff \text{constrained}(A) \wedge \text{choice}(A)$ | Agent A is free if and only if A is constrained and A has choice | Freedom within constraint |
| C₅₇ | Normative | 7 | $\forall A: \text{telos}(A) = \text{generativity}$ | For all agents A, the telos of A is generativity | Generativity as ethical telos |
| C₅₈ | Normative | 7 | $\exists v₁, v₂: \text{value}(v₁) \wedge \text{value}(v₂) \wedge \text{incommensurable}(v₁, v₂)$ | There exist values v₁ and v₂ such that both are genuine values yet are incommensurable | Value pluralism |
| C₅₉ | Normative | 7 | $\forall A₁, A₂: \text{equals}(A₁, A₂) \implies \text{equal-treatment}(A₁, A₂)$ | For all agents A₁ and A₂, if A₁ equals A₂, then A₁ and A₂ deserve equal treatment | Justice |
| C₆₀ | Normative | 7 | $\forall A₁, A₂: \text{recognize}(A₁, A₂)$ required | For all agents A₁ and A₂, A₁ must recognize A₂ | Recognition |
| C₆₁ | Modal | 8 | $\exists \phi: \Box \phi$ | There exists a proposition φ such that φ is necessarily true | Necessity |
| C₆₂ | Modal | 8 | $\exists \phi: \Diamond \phi$ | There exists a proposition φ such that φ is possibly true | Possibility |
| C₆₃ | Modal | 8 | $\exists \phi: \neg\Box\phi \wedge \neg\Box\neg\phi$ | There exists a proposition φ such that φ is neither necessarily true nor necessarily false | Contingency |
| C₆₄ | Modal | 8 | $\forall e₁, e₂: e₁ \square\!\rightarrow e₂ \iff (\neg e₁ \implies \neg e₂)$ | For all events e₁ and e₂, e₁ counterfactually depends on e₂ if and only if not-e₁ implies not-e₂ | Counterfactual dependence |
| C₆₅ | Modal | 8 | $\exists w₁, w₂, w₃: w₁ \text{ accesses } w₂ \text{ accesses } w₃$ | There exist worlds w₁, w₂, w₃ such that w₁ accesses w₂ and w₂ accesses w₃ | Modal depth |
| C₆₆ | Modal | 8 | $\text{[Reserved for future modal condition]}$ | [Reserved for future modal condition] | [Reserved] |
| C₆₇ | Phenomenological | 9 | $\forall x: \text{appears}(x) \implies \text{given}(x)$ | For all x, if x appears, then x is given | Givenness |
| C₆₈ | Phenomenological | 9 | $\forall \text{consciousness}(c): \exists x: \text{directed-at}(c, x)$ | For all consciousness c, there exists x such that c is directed at x | Intentionality |
| C₆₉ | Phenomenological | 9 | $\forall \text{experience}(e): \text{affective-tone}(e)$ | For all experiences e, e has an affective tone | Affectivity |
| C₇₀ | Phenomenological | 9 | $\forall \text{subject}(s): \text{embodied}(s)$ | For all subjects s, s is embodied | Embodiment |
| C₇₁ | Phenomenological | 9 | $\forall \text{experience}(e): \text{temporal-structure}(e)$ | For all experiences e, e has temporal structure | Temporality of experience |
| C₇₂ | Phenomenological | 9 | $\exists s₁, s₂: \text{shared-affect}(s₁, s₂)$ | There exist subjects s₁ and s₂ such that s₁ and s₂ share affect | Interaffectivity |
| C₇₃ | Systemic | 10 | $\forall S: \exists E: \text{distinct}(S, E)$ | For every system S, there exists an environment E distinct from S | System-environment distinction |
| C₇₄ | Systemic | 10 | $\forall S: \text{self-produce}(S) \wedge \text{self-maintain}(S)$ | For every system S, S self-produces and self-maintains | Autopoiesis |
| C₇₅ | Systemic | 10 | $\forall S: \exists F: \text{feedback}(F, S)$ | For every system S, there exists a feedback loop F for S | Feedback loops |
| C₇₆ | Systemic | 10 | $\forall S, p: \text{perturbation}(p, S) \implies \text{recovers}(S)$ possible | For all systems S and perturbations p, if p perturbs S, then S recovering is possible | Resilience |
| C₇₇ | Systemic | 10 | $\forall S, e: \text{environment-change}(e) \implies \text{adapt}(S, e)$ possible | For all systems S and environmental changes e, if e occurs, then S adapting to e is possible | Adaptability |
| C₇₈ | Systemic | 10 | $\exists L₁, L₂, \ldots, L_n: L_i \subseteq L_{i+1}$ | There exist levels L₁, L₂, ..., Lₙ such that each level is contained in the next | Nested hierarchy |
| C₇₉ | Systemic | 10 | $\forall t: \text{complexity}(S, t+1) \geq \text{complexity}(S, t)$ | For all times t, the complexity of S at t+1 is greater than or equal to its complexity at t | Open-ended evolution |
